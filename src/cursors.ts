...
> implements AsyncDisposable {
	...
	_actuallyClosed = false;
	otherMutex = new AsyncMutex3();

	outOfBandError: unknown = null;
	outOfBandErrorSet = false;
	...
	get closed(): boolean {
		return !!this.closePromise || this._actuallyClosed;
	}
	...
	_ensureNotErrored() {
		if (this.outOfBandErrorSet) {
			throw this.outOfBandError;
		}
	}
	...
	async runPump() {
		try {
			...
			if (this.pendingRequests.length > 0 || !this._actuallyClosed) {
