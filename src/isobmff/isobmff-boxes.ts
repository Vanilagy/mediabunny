...
import { type MetadataTags, RichImageData } from '../metadata'
import {
  assert,
  assertNever,
  COLOR_PRIMARIES_MAP,
...
import {
  GLOBAL_TIMESCALE,
  getTrackMetadata,
  type IsobmffAudioTrackData,
  type IsobmffMuxer,
  type IsobmffSubtitleTrackData,
  type IsobmffTrackData,
  type IsobmffVideoTrackData,
  intoTimescale,
  type Sample,
} from './isobmff-muxer'
    ...
    if (box.contents && !box.children) {
      ...
      if (box.children)
        for (const child of box.children)
          if (child) result += this.measureBox(child)
...
export const mdat = (reserveLargeSize: boolean): Box => ({
  type: 'mdat',
...
export const mvhd = (creationTime: number, trackDatas: IsobmffTrackData[]) => {
...
export const tkhd = (trackData: IsobmffTrackData, creationTime: number) => {
...
export const mdia = (trackData: IsobmffTrackData, creationTime: number) =>
  box('mdia', undefined, [
    mdhd(trackData, creationTime),
    hdlr(
      true,
      TRACK_TYPE_TO_COMPONENT_SUBTYPE[trackData.type],
      TRACK_TYPE_TO_HANDLER_NAME[trackData.type],
    ),
    minf(trackData),
  ])

/** Media Header Box: Specifies the characteristics of a media, including timescale and duration. */
export const mdhd = (trackData: IsobmffTrackData, creationTime: number) => {
  const lastSample = lastPresentedSample(trackData.samples)
  ...
  return fullBox('mdhd', +needsU64, 0, [
    ...
    u16(
      getLanguageCodeInt(
        trackData.track.metadata.languageCode ?? UNDETERMINED_LANGUAGE,
      ),
    ), // Language
    u16(0), // Quality
  ])
...
const TRACK_TYPE_TO_COMPONENT_SUBTYPE: Record<
  IsobmffTrackData['type'],
  string
> = {
  video: 'vide',
  audio: 'soun',
 ...
 */
export const minf = (trackData: IsobmffTrackData) =>
  box('minf', undefined, [
 ...
 */
export const dinf = () => box('dinf', undefined, [dref()])
...
export const stbl = (trackData: IsobmffTrackData) => {
  const needsCtts =
    trackData.compositionTimeOffsetTable.length > 1 ||
    trackData.compositionTimeOffsetTable.some(
...
export const videoSampleDescription = (
  ...
  box(
    ...
    [
      VIDEO_CODEC_TO_CONFIGURATION_BOX[trackData.track.source._codec](
        trackData,
      ),
      colorSpaceIsComplete(trackData.info.decoderConfig.colorSpace)
        ? colr(trackData)
        : null,
    ],
  )
...
export const colr = (trackData: IsobmffVideoTrackData) =>
  box('colr', [
    ...
    u16(
      COLOR_PRIMARIES_MAP[trackData.info.decoderConfig.colorSpace!.primaries!],
    ), // Colour primaries
    u16(
      TRANSFER_CHARACTERISTICS_MAP[
        trackData.info.decoderConfig.colorSpace!.transfer!
      ],
    ), // Transfer characteristics
    u16(
      MATRIX_COEFFICIENTS_MAP[trackData.info.decoderConfig.colorSpace!.matrix!],
    ), // Matrix coefficients
    u8((trackData.info.decoderConfig.colorSpace!.fullRange ? 1 : 0) << 7), // Full range flag
  ])

/** AVC Configuration Box: Provides additional information to the decoder. */
export const avcC = (trackData: IsobmffVideoTrackData) =>
...
export const hvcC = (trackData: IsobmffVideoTrackData) =>
  trackData.info.decoderConfig &&
  box('hvcC', [
...
export const vpcC = (trackData: IsobmffVideoTrackData) => {
  ...
  const videoFullRangeFlag = parts[8]
    ? Number(parts[8])
    : Number(decoderConfig.colorSpace?.fullRange ?? 0)
  const thirdByte =
...
export const soundSampleDescription = (
  ...
  if (
    (PCM_AUDIO_CODECS as readonly AudioCodec[]).includes(
      trackData.track.source._codec,
    )
  ) {
...
export const enda = (trackData: IsobmffAudioTrackData) => {
  const { littleEndian } = parsePcmCodec(
    trackData.track.source._codec as PcmAudioCodec,
  )
...
export const dfLa = (trackData: IsobmffAudioTrackData) => {
  ...
  return fullBox('dfLa', 0, 0, [...bytes.subarray(4)])
}

/** PCM Configuration Box, ISO/IEC 23003-5. */
const pcmC = (trackData: IsobmffAudioTrackData) => {
  const { littleEndian, sampleSize } = parsePcmCodec(
    trackData.track.source._codec as PcmAudioCodec,
  )
...
export const subtitleSampleDescription = (
  ...
  box(
    ...
    [
      SUBTITLE_CODEC_TO_CONFIGURATION_BOX[trackData.track.source._codec](
        trackData,
      ),
    ],
  )

export const vttC = (trackData: IsobmffSubtitleTrackData) =>
  box('vttC', [...textEncoder.encode(trackData.info.config.description)])

export const txtC = (textConfig: Uint8Array) =>
  fullBox('txtC', 0, 0, [
    ...textConfig,
    0, // Text config (null-terminated)
  ])
...
export const stss = (trackData: IsobmffTrackData) => {
  ...
  const keySamples = [...trackData.samples.entries()].filter(
    ([, sample]) => sample.type === 'key',
  )
...
export const stsz = (trackData: IsobmffTrackData) => {
  if (trackData.type === 'audio' && trackData.info.requiresPcmTransformation) {
    const { sampleSize } = parsePcmCodec(
      trackData.track.source._codec as PcmAudioCodec,
    )
    ...
    return fullBox('stsz', 0, 0, [
      ...
      u32(
        trackData.samples.reduce(
          (acc, x) => acc + intoTimescale(x.duration, trackData.timescale),
          0,
        ),
      ),
    ])
...
export const stco = (trackData: IsobmffTrackData) => {
  if (
    trackData.finalizedChunks.length > 0 &&
    last(trackData.finalizedChunks)!.offset! >= 2 ** 32
  ) {
...
export const cslg = (trackData: IsobmffTrackData) => {
  ...
  for (let i = 0; i < trackData.compositionTimeOffsetTable.length; i++) {
    ...
    leastDecodeToDisplayDelta = Math.min(
      leastDecodeToDisplayDelta,
      entry.sampleCompositionTimeOffset,
    )
    greatestDecodeToDisplayDelta = Math.max(
...
export const moof = (
  ...
  return box('moof', undefined, [mfhd(sequenceNumber), ...trackDatas.map(traf)])
}
...
export const tfhd = (trackData: IsobmffTrackData) => {
  ...
  const referenceSample =
    trackData.currentChunk.samples[1] ?? trackData.currentChunk.samples[0]!
...
export const tfdt = (trackData: IsobmffTrackData) => {
  ...
  return fullBox('tfdt', 1, 0, [
    u64(
      intoTimescale(trackData.currentChunk.startTimestamp, trackData.timescale),
...
export const trun = (trackData: IsobmffTrackData) => {
  ...
  const allSampleDurations = trackData.currentChunk.samples.map(
    (x) => x.timescaleUnitsToNextSample,
  )
  const allSampleSizes = trackData.currentChunk.samples.map((x) => x.size)
  const allSampleFlags = trackData.currentChunk.samples.map(fragmentSampleFlags)
  const allSampleCompositionTimeOffsets = trackData.currentChunk.samples.map(
    (x) => intoTimescale(x.timestamp - x.decodeTimestamp, trackData.timescale),
  )

  const uniqueSampleDurations = new Set(allSampleDurations)
  const uniqueSampleSizes = new Set(allSampleSizes)
  const uniqueSampleFlags = new Set(allSampleFlags)
  const uniqueSampleCompositionTimeOffsets = new Set(
    allSampleCompositionTimeOffsets,
  )

  const firstSampleFlagsPresent =
    uniqueSampleFlags.size === 2 && allSampleFlags[0] !== allSampleFlags[1]
  const sampleDurationPresent = uniqueSampleDurations.size > 1
  const sampleSizePresent = uniqueSampleSizes.size > 1
  const sampleFlagsPresent =
    !firstSampleFlagsPresent && uniqueSampleFlags.size > 1
  const sampleCompositionTimeOffsetsPresent =
    uniqueSampleCompositionTimeOffsets.size > 1 ||
  ...
  return fullBox('trun', 1, flags, [
    ...
    u32(
      trackData.currentChunk.offset! - trackData.currentChunk.moofOffset! || 0,
    ), // Data offset
...
export const mfra = (trackDatas: IsobmffTrackData[]) => {
  return box('mfra', undefined, [...trackDatas.map(tfra), mfro()])
}
...
export const vttc = (
  ...
  box('vttc', undefined, [
    ...
    identifier !== null
      ? box('iden', [...textEncoder.encode(identifier)])
      : null,
    timestamp !== null
      ? box('ctim', [...textEncoder.encode(formatSubtitleTimestamp(timestamp))])
      : null,
...
export const vtta = (notes: string) =>
  box('vtta', [...textEncoder.encode(notes)])
...
const udta = (muxer: IsobmffMuxer) => {
  ...
  if (
    metadataFormat === 'mdir' ||
    (metadataFormat === 'auto' && !muxer.isQuickTime)
  ) {
  ...
  for (const { key, value } of keyValueIterator(tags)) {
    switch (key) {
      ...
      case 'date':
        {
          boxes.push(
            metadataTagStringBoxShort('©day', value.toISOString().slice(0, 10)),
          )
        }
        break
...
const metaMdir = (tags: MetadataTags) => {
  ...
  return fullBox('meta', 0, 0, undefined, [
    ...
    box(
      'ilst',
      undefined,
      pairs.map((pair) => box(pair.key, undefined, [pair.value])),
...
const VIDEO_CODEC_TO_CONFIGURATION_BOX: Record<
  VideoCodec,
  (trackData: IsobmffVideoTrackData) => Box | null
> = {
  avc: avcC,
  hevc: hvcC,
  vp8: vpcC,
...
const audioCodecToConfigurationBox = (
  codec: AudioCodec,
  isQuickTime: boolean,
) => {
  switch (codec) {
