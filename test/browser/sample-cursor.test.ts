...
test.only('Use after close', async () => {
	using input = new Input({
		source: new UrlSource('/trim-buck-bunny.mov'),
		formats: ALL_FORMATS,
	});
	...
	expect(() => cursor2.next()).toThrow('cursor has been closed');

	await expect(commands[0]!).resolves.toBeInstanceOf(VideoSample);
	await expect(commands[1]!).resolves.toBeInstanceOf(VideoSample);
	await expect(commands[2]!).resolves.toBeUndefined();
});
...
test('Automatic cursor disposal', async () => {
	...
	expect(cursor._actuallyClosed).toBe(true);
});
